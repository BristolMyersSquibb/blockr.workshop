...
:::{.column}
:::{.incremental}
1. Click on `Add stack`, then add it a `customdata_block` with `lab` data. Change the title to `Lab data`.


2. Click on `Add stack`, then add a `customdata_block` with `demo` data: 

    a. Add a `join_block` with `y = "lab_data"`, `type = "inner"`, `by = c("STUDYID", "USUBJID")`.
    b. Add a `filter_block` with `columns = "LBTEST"`, `values = "Hemoglobin"`.
    c. Add an `arrange_block` with `VISITNUM`.
    d. Add a `mutate_block` with the following expression: `VISIT = factor(VISIT, levels = unique(VISIT), ordered = TRUE)`.
    e. Rename the stack to `Hemoglobin data`.


3. Click on `Add stack`, then add a `result_block` selecting the `Hemoglobin_data` stack:

    a. Add a `group_by_block` with `"VISIT` and `ACTARM` as columns.
    b. Add a `summarize_block` with `func = c("mean", "se")`, `default_columns = c("LBSTRESN", "LBSTRESN")`.
    c. Add an `ungroup_block`.
    d. Add a `mutate_block` with `ymin = "MEAN -SE"` and then click `+` to add another expression, `ymax = "MEAN + SE"`.
    e. Change the stack title to `Summary data`.
:::
...
:::{.column}
:::{.incremental} 
4. Click on `Add stack`, then add a `result_block`, targeting the `Summary data` stack:
